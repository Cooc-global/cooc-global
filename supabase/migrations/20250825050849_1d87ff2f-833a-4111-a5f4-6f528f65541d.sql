-- Fix the daily returns functions by removing STABLE keyword and allowing modifications
...
BEGIN
  ...
  LOOP
    -- Check if return already processed for today - FIXED: use CURRENT_DATE instead of return_date
    IF NOT EXISTS (
      ...
      -- Log the return processing for developer tracking
  ...
  LOOP
    ...
    IF developer_id IS NOT NULL THEN
      ...
      -- Record developer capital acquisition transaction
      INSERT INTO public.transactions (
        user_id, 
        transaction_type, 
        amount, 
    ...
    -- Record completion transaction for investor
    INSERT INTO public.transactions (
      user_id, 
      transaction_type, 
      amount, 
